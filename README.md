...
- **Live and shared** (backed by the `AVAILABILITY` KV namespace, visible to every admin and to the public site where relevant): Calendar & Booking, Bookings Overview, Messages, and Contact Info/Social Links. Contact Info is read/written through `/api/admin/content` (Access-protected, `functions/api/admin/content.js`) under the `siteContact` KV key; the homepage reads the same key back through the public, unauthenticated `/api/site-content` (`functions/api/site-content.js`) and fills in the email/location text and social links on load — see the fetch near the top of `script.js`. `localStorage` is kept only as an instant same-browser pre-fill while the server request is in flight.
- **Local drafts only** (saved to `localStorage` in whichever browser you're using): Services, Gallery, and About. There's no content backend for these yet, so edits here are a draft you can review and copy from — they do **not** change the live homepage by themselves. To publish a change, edit the matching text directly in `index.html` (e.g. a service description, the About paragraphs) and redeploy the site. If you'd like these to also save live and update the homepage automatically, that's a follow-up project similar to how Contact Info was just wired up.
