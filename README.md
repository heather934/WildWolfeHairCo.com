...
- **Live and shared** (visible to every admin and to the public site where relevant): Calendar & Booking, Bookings Overview, Messages, **Gallery**, and **Contact Info/Social Links**. Calendar/Booking and Messages are backed by the `AVAILABILITY` KV namespace; Gallery photos are stored in the `GALLERY_BUCKET` R2 bucket, with their captions/order in the same `AVAILABILITY` KV namespace (`gallery` key) — uploading a photo in the admin panel makes it appear on the public homepage gallery immediately, no redeploy needed. Contact Info is read/written through `/api/admin/content` (Access-protected, `functions/api/admin/content.js`) under the `siteContact` key in the same KV namespace; the homepage reads it back through the public, unauthenticated `/api/site-content` (`functions/api/site-content.js`) and fills in the email/location text and social links on load — see the fetch near the top of `script.js`.
- **Local drafts only** (saved to `localStorage` in whichever browser you're using): Services and About. There's no content backend for these yet, so edits here are a draft you can review and copy from — they do **not** change the live homepage by themselves. To publish a change, edit the matching text directly in `index.html` (e.g. a service description, the About paragraphs) and redeploy the site. If you'd like these to also save live and update the homepage automatically, that's a follow-up project similar to how Gallery and Contact Info were wired up.

### Gallery

`functions/api/admin/gallery.js` (Access-protected) handles uploads and deletes: photo bytes go to the `GALLERY_BUCKET` R2 bucket, and captions/order go to the `gallery` key in the `AVAILABILITY` KV namespace. `functions/api/gallery/index.js` and `functions/api/gallery/[id].js` are the public (unauthenticated) read routes the homepage's `gallery-script.js` fetches from — the same data, minus internal fields. Before a photo is ever uploaded, `admin-script.js` decodes it in the browser and re-encodes it as a downscaled JPEG (max 1600px, quality 0.85): this keeps uploads small and fast regardless of the original phone photo's size, and — importantly — means an iPhone's HEIC photo becomes a JPEG that renders in every visitor's browser, not just Safari. If a photo can't be decoded client-side within 20 seconds (e.g. it's still downloading from iCloud on an "Optimize iPhone Storage" device) or can't be decoded at all, the upload form shows a clear error instead of hanging.

### Contact Info

`functions/api/admin/content.js` (Access-protected) reads/saves email, phone, location, response message, and social links under the `siteContact` key in the `AVAILABILITY` KV namespace. `functions/api/site-content.js` is the public (unauthenticated) read route the homepage fetches from on load to fill in the email/location text and social link URLs, falling back to the hardcoded text already in `index.html` if nothing's been saved yet or the fetch fails. `localStorage` is kept only as an instant same-browser pre-fill for the admin form while the server request is in flight — it is not the source of truth.
