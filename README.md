...
- **Live and shared** (visible to every admin and to the public site where relevant): Calendar & Booking, Bookings Overview, Messages, and **Gallery**. Calendar/Booking and Messages are backed by the `AVAILABILITY` KV namespace; Gallery photos are stored in the `GALLERY_BUCKET` R2 bucket, with their captions/order in the same `AVAILABILITY` KV namespace (`gallery` key). Uploading a photo in the admin panel makes it appear on the public homepage gallery immediately — no redeploy needed.
- **Local drafts only** (saved to `localStorage` in whichever browser you're using): Services, About, and Contact Info/Social Links. There's no content backend for these yet, so edits here are a draft you can review and copy from — they do **not** change the live homepage by themselves. To publish a change, edit the matching text directly in `index.html` (e.g. a service description, the About paragraphs, the footer contact info) and redeploy the site. If you'd like these to also save live and update the homepage automatically, that's a follow-up project similar to how booking/availability and the gallery were wired up.
